...
class StructuredDatasetV170(Dataset):
    def __init__(self, arrays: StructuredArraysV170) -> None:
        self.arrays = arrays
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        return {
            ...
            "future_target": torch.from_numpy(self.arrays.future_targets[index]),
...
def _ssl_targets(
    ...
    """History reconstruction plus target-relevant future distribution tasks.

    Future targets are used only for training origins. They are never supplied to
    the encoder and therefore do not leak into validation or test features.
    """
    ...
    future = np.asarray(future_returns, dtype=np.float32)
    future_descriptors = np.concatenate(
        [future, np.abs(future), np.minimum(future, 0.0)], axis=1
    ).astype(np.float32)
    return reconstruction, descriptors, future_descriptors


def build_structured_arrays_v170(
    ...
    ssl_target, regime_target, future_target = _ssl_targets(histories, targets)
    return StructuredArraysV170(
        histories=histories,
...
class SSLRegimePatchTransformer(nn.Module):
    ...
    def __init__(self, config: StructuredV170Config) -> None:
        super().__init__()
        self.config = config
        ...
        self.future_head = nn.Sequential(
            nn.Linear(config.d_model, config.d_model // 2),
            nn.GELU(),
            nn.Linear(config.d_model // 2, config.future_target_dim),
        )
    ...
    def ssl_outputs(
        ...
        return (
            ...
            self.future_head(representation),
            representation,
        )
...
def _model_config(arrays: StructuredArraysV170, settings: dict[str, Any], horizons: list[int], quantiles: list[float]) -> StructuredV170Config:
    return StructuredV170Config(
        ...
        future_target_dim=arrays.future_targets.shape[1],
...
def _groups(panel: LongHistoryPanel, fold: Fold, config: dict[str, Any], lookback: int) -> dict[str, np.ndarray]:
    return split_long_origins(
        ...
        common_max_horizon=(
            ...
            if bool(config["performance_v170"].get("common_origin_required", True))
            else None
        ),
    )


def pretrain_structured_fold_v170(
    panel: LongHistoryPanel,
    fold: Fold,
    config: dict[str, Any],
    statistical_state: StatisticalEnsembleState,
    ...
    settings = config["performance_v170"]["structured_model"]
    ...
    arrays = build_structured_arrays_v170(
        panel,
        groups["train"][::stride],
        horizons,
        quantiles,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in loader:
            ...
            reconstruction_a, regime_a, future_a, representation_a = model.ssl_outputs(
                history_tensor, float(settings["mask_probability"])
            )
            reconstruction_b, regime_b, future_b, representation_b = model.ssl_outputs(
                history_tensor, float(settings["mask_probability"])
            )
            ...
            future_loss = 0.5 * (
                F.smooth_l1_loss(future_a, future_target)
                + F.smooth_l1_loss(future_b, future_target)
            )
            ...
            loss = (
                ...
                + float(settings["pretrain_future_weight"]) * future_loss
        ...
        print(
            f"{fold.name} structured-v170-pretrain seed={seed} "
...
def train_structured_fold_v170(
    panel: LongHistoryPanel,
    fold: Fold,
    config: dict[str, Any],
    statistical_state: StatisticalEnsembleState,
    ...
    train_arrays = build_structured_arrays_v170(
        panel,
        groups["train"][::stride],
        horizons,
        quantiles,
    ...
    validation_arrays = build_structured_arrays_v170(
        panel,
        groups["validation"],
        horizons,
        quantiles,
    ...
    model = SSLRegimePatchTransformer(StructuredV170Config(**model_config)).to(device)
    ...
    train_loader = DataLoader(
        StructuredDatasetV170(train_arrays),
        batch_size=int(settings["batch_size"]),
        shuffle=True,
        num_workers=int(settings.get("num_workers", 0)),
        pin_memory=device.type == "cuda",
    ...
    validation_loader = DataLoader(
        StructuredDatasetV170(validation_arrays),
        batch_size=int(settings["batch_size"]),
        shuffle=False,
        num_workers=int(settings.get("num_workers", 0)),
        pin_memory=device.type == "cuda",
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            ...
            baseline_loss = weighted_pinball_loss(
            ...
            loss = (
                loss
                + float(settings.get("baseline_guard_weight", 0.25))
                * F.relu(loss - baseline_loss)
        ...
        print(
            f"{fold.name} structured-v170 seed={seed} epoch={epoch:03d} "
        ...
        if validation_loss < best - 1e-10:
            ...
            if wait >= int(settings["patience"]):
                break
    if best_state is None:
        raise RuntimeError("Structured v1.7 model produced no checkpoint")
    output = Path(output_dir)
    output.mkdir(parents=True, exist_ok=True)
    torch.save(
        {
    ...
    write_json(
        output / "run_metadata.json",
        {
            **runtime_metadata(seed, device),
            "fold": asdict(fold),
            "model_family": "ssl_regime_patch_transformer_v170",
            "best_epoch": int(best_epoch),
            "best_validation_loss": float(best),
...
def load_structured_checkpoint_v170(
    path: str | Path,
    device_name: str = "auto",
) -> tuple[
    ...
    model = SSLRegimePatchTransformer(StructuredV170Config(**record)).to(device)
    model.load_state_dict(checkpoint["model"], strict=True)
    model.eval()
    return (
        model,
...
def predict_structured_v170(
    model: SSLRegimePatchTransformer,
    arrays: StructuredArraysV170,
    device: torch.device,
    batch_size: int,
) -> np.ndarray:
    loader = DataLoader(
        StructuredDatasetV170(arrays), batch_size=batch_size, shuffle=False
    )
...
def export_structured_seed_v170(
    panel: LongHistoryPanel,
    fold: Fold,
    config: dict[str, Any],
    ...
    started = time.time()
    model, spec, statistical_state, checkpoint, device = load_structured_checkpoint_v170(
