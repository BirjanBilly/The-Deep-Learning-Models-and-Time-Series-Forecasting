...
def refined_tail_config_v170(
    config: dict[str, Any],
    objective: str,
) -> dict[str, Any]:
    refined = copy.deepcopy(config)
    base = refined["performance_v170"]["tail_model"]
    settings = copy.deepcopy(base)
    overrides = base.get("family_overrides", {}).get(objective, {})
    settings.update(overrides)
    refined["tailrisk"].update(
        {
            "output_dir": refined["performance_v170"]["output_dir"],
...
def export_tail_seed_predictions_v170(
    panel: Panel,
    fold: Fold,
    config: dict[str, Any],
    ...
    # its family-local baseline by the v1.7 statistical expert ensemble.
    adjusted = copy.deepcopy(config)
    adjusted["performance_v150"]["tail_factor_scale"] = copy.deepcopy(
        config["performance_v170"]["tail_model"]
    )
    ...
    write_json(
        output / "v170_export_metadata.json",
        {
            "model_family": "stable_factor_scale_v170",
