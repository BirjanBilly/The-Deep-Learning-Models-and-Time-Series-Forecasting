...
from riskgraph.performance_v170.structured import pretrain_structured_fold_v170


def main() -> None:
    parser = argparse.ArgumentParser(description="Self-supervised pretrain v1.7 structured encoder")
    ...
    for fold in selected_folds(config, args.folds):
        ...
        for seed in args.seeds:
            ...
            if args.skip_existing and (output / "pretrain_checkpoint.pt").is_file():
                print(f"skipping existing pretrain -> {output}")
                continue
            pretrain_structured_fold_v170(panel, fold, config, stat, seed, output, args.device, args.epochs)
